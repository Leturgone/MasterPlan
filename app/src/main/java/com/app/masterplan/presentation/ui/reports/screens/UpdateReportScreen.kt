...
fun UpdateReportScreen(
    ...
    viewModel: UpdateReportViewModel = hiltViewModel(),
) {

    val reportData: UpdateReportData? by viewModel.updatedReport.collectAsState()
    ...
    Box(modifier = Modifier.fillMaxSize()){
        ...
        Column(
            ...
            LaunchedEffect(Unit) {
                viewModel.loadReport(reportId,reportType)
            }
            reportData?.let {
                Text(
                    text = when(reportType){
                        ReportType.TASK -> stringResource(id = R.string.update_report_for_tasks)
                        ReportType.PLAN -> stringResource(id = R.string.update_report_for_plans)
                    },
