...
@HiltViewModel
class EmployeeListScreenViewModel @Inject constructor (
    ...
    private val _downloadExport = MutableStateFlow<MasterPlanState<File>>(MasterPlanState.Waiting)

    val downloadExport: StateFlow<MasterPlanState<File>> = _downloadExport
    ...
    fun exportDirEmployees() = viewModelScope.launch {
        _downloadExport.value = MasterPlanState.Loading

        val exportedFile = exportDirEmployeesUseCase(directorId).getOrElse {
            _downloadExport.value =  MasterPlanState.Failure(Exception(it.message))
            return@launch
        }
        _downloadExport.value = MasterPlanState.Success(exportedFile)
    }
}